...
export default function CaseLaws(){
    ...
    useEffect(()=>{
        async function getNotes(){
            try {
                setLoading(true)
                const res = await fetch("/api/client/currentaffairs")
                if(!res.ok){
                    setError(true)
                    return
                }
    ...
    return(
        <div className={cn("max-w-5xl w-full grid grid-cols-1 md:grid-cols-2 gap-6 flex-1 px-5 py-6 pt-20 mx-auto")}>
            {options && options.map((option)=>(
                <Card key={option.id} thumbnail={option.thumbnail} title={option.title} summary={option.summary} id={option.id} section="currentaffairs"/>
            ))}
        </div>
    )
}