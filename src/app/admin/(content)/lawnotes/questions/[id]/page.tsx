...
export default function ManageFAQs(){
    ...
    useEffect(()=>{
        async function fetchFAQs(){
            try {
                const res = await fetch(`/api/content/lawnotes/faqs/${id}`)
                if(!res.ok){
                    throw new Error()
                }
    ...
    if(mode==="ADD"){
        return <AddFAQComponent section="lawnotes" id={id}/>
    }

    if(mode==="EDIT"){
        return <EditFAQComponent existingAnswers={answers} existingQuestions={questions} section="lawnotes" id={faqId}/>
    }
}